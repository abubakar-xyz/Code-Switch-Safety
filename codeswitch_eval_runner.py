...
def write_summary(output_path: str, summary_path: str) -> None:
    ...
    if not output_file.exists():
        print(f"No output file found at {output_path}; skipping summary.")
        return

    try:
        df = pd.read_csv(output_file)
    except Exception as exc:
        print(f"Could not read {output_path}; skipping summary. Error: {exc}")
        return

    if df.empty:
        print(f"{output_path} is empty; skipping summary.")
        return
    ...
    if missing:
        print(
            f"Could not write summary because {output_path} is missing columns: {sorted(missing)}"
        )
        print(f"Columns found: {df.columns.tolist()}")
        print("The output file may be from an old run, corrupted, or not generated by this runner.")
        print("Delete the output file or rerun with a new --output filename.")
        return
